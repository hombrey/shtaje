...
if [[ $isShuffled == "y" ]]; then
        ...
        shuf $LESSONDIR/rawlist.txt > $LESSONDIR/list.txt
else
        cp $LESSONDIR/rawlist.txt $LESSONDIR/list.txt
fi
...
rm $LESSONDIR/rawlist.txt

# n option should prevent from overwriting
cp -nr $SOURCEDIR/choices . 
